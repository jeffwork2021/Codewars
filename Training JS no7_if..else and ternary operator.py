...
# otherwise execute the statementb. If the statementa or statementb are more 
# than one line, you need to add { and } at the head and tail of statements in 
# JS, to keep the same indentation on Python and to put an end in Ruby where it # indeed ends.
#
# For example, if we want to judge whether a number is odd or even, we can # 
...
#
# If there is more than one condition to judge, we can use the compound if...
...
# parameter:n, n is the number of hotdogs a customer will buy, different 
# numbers have different prices (refer to the following table), return how much 
...
def sale_hotdogs(n):
    return n*100 if n<5 else n*90 if n >= 10 else n*95