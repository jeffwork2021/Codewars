...
# beginning with W.
#
# If L or M are empty return string is "" (Clojure/Racket/Prolog should return 
...
def stock_list(list_of_art, list_of_cat):
    if not list_of_art or not list_of_cat:
        return ""
    keys = list(set(list(map(lambda x: x[0], list_of_art)) + \
                    list(map(lambda x: x, list_of_cat))))
    counter = dict.fromkeys(keys, 0)
    for book in list_of_art:
        counter[book[0]] += int(book.split()[1])
    return ' - '.join(f'({cat} : {counter[cat]})' for cat in list_of_cat)
