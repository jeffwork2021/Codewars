...
# the screen, using asterisk (*) characters. Trailing spaces should be removed, # and every line must be terminated with a newline character (\n).
#
...
def diamond(n):
    if not n or n < 0 or n % 2 == 0:
        return None
    return ''.join(('*'*i).center(n).rstrip() + '\n' 
        for i in list(range(1,n+1,2)) + list(range(n-2,0,-2)))
