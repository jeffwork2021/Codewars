...
# Example:
#
# With list ls and 3 towns to visit they can make a choice between: [50,55,57],
...
# nil, null, None, Nothing, depending on the language. In that case with C, C
# ++, D, Dart, Fortran, F#, Go, Julia, Kotlin, Nim, OCaml, Pascal, Perl, 
...
def choose_best_sum(t, k, ls):
    max_sum = -1
    for p in combinations(ls, k):
        curr_sum = sum(p)
        if t >= curr_sum > max_sum:
            max_sum = curr_sum
    return None if max_sum == -1 else max_sum