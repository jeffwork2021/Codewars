...
# Kings'
# title_case('THE WIND IN THE WILLOWS', 'The In') # should return: 'The Wind in 
# the Willows'
# title_case('the quick brown fox') # should return: 'The Quick Brown Fox'
def title_case(title, minor_words=''):
    arr = []
    check_tokens = minor_words.lower().split()
    for ind, token in enumerate(title.lower().split()):
        if ind < 1 or token not in check_tokens:
